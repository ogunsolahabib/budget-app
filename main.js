...
const budgetController = (function() {
  ...
  class Expense {
    ...
    calcPercentage() {
      if (data.totals.inc > 0) {
        this.percentage = Math.floor((this.value / data.totals.inc) * 100);
      } else {
        this.percentage = -1;
      }
    }
  ...
  return {
    ...
    deleteItem: (type, id) => {
      ...
      index = ids.findIndex(cur => cur === id);
      if (index !== -1) {
...
const UIController = (() => {
  ...
  return {
    ...
    displayBudget: obj => {
      ...
      ).textContent = formatNumber(obj.expense, 'exp');
      if (obj.percentage > 0) {
    ...
    displayPercentages: array => {
      percentagesNodeList = document.querySelectorAll(
        DOMstrings.expensePercentage
      );
      fields = Array.from(percentagesNodeList);
